...
const Index = () => {
  ...
  const { HeaderInfo, setLanguage } = useContext(AppContext)
    ...
    <header className='w-full bg-white drop-shadow-md text-[#183B56] fixed top-0 left-0 z-[999]'>
      <div className={styles.header}>
        ...
        <div className='flex items-center'>
          ...
          <span className='ml-4 text-xl font-semibold hidden md:block'>Laeeb</span>
        </div>
        ...
        <div className='flex items-center'>
          <div className='relative flex items-center cursor-pointer p-3 select-none text-sm' ref={langMuneRef} onClick={()=>setLangMuneShow(!langMuneShow)}>
            ...
            <AiOutlineDown />
            <ul className={`absolute top-10 bg-white shadow-md transition-all duration-300 border-t border-gray-100 ${!langMuneShow && styles.UlHidden}`}>
              <li className={langClass} onClick={()=>chosseLang('English')}>English</li>
              <li className={langClass} onClick={()=>chosseLang('中文')}>中文</li>
            </ul>
          </div>
          <div className='bg-[#ee574c] btn md:px-6 ml-3 text-sm' onClick={()=>Message.onInfo('即将开放')}>{HeaderInfo.btn}</div>
